    ...
    <div style="border: 1px solid #EBEEF5;border-bottom: none;background: #EBEEF5">
        <el-form :inline="true" style="padding: 5px">
            <div class="tabTool">
                ...
                <div v-show="tool" class="tabToolBox">
                    <el-tooltip class="item" effect="dark" content="导出" placement="bottom-start">
                        <el-button icon="el-icon-download" circle @click="this.$parent.reFresh"
                        ></el-button>
                    </el-tooltip>
                    <el-tooltip class="item" effect="dark" content="打印" placement="bottom-start">
                        <el-button icon="el-icon-printer" circle @click="this.$parent.reFresh"
                        ></el-button>
                    </el-tooltip>
                    <el-tooltip class="item" effect="dark" content="刷新" placement="bottom-start">
                        <el-button icon="el-icon-refresh" circle @click="this.$parent.reFresh"
                        ></el-button>
                    </el-tooltip>
                    <el-popover
                            placement="bottom"
...
<script>
  ...
  export default {
    ...
    computed: {
      searchItem: {
        get: function () {
          return this.$parent.$store.state.search.item
        }
      }
    },
    mixins: [LpBase],
    ...
    methods: {
      ...
      resetSearchForm() {
        this.$parent.$store.commit("reset", "search")
        this.$parent.$store.dispatch("loadTable")
      },
      extraSearch() {
        this.ovtation = !this.ovtation;
